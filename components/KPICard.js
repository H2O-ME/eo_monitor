...
export default function KPICard({ title, value, description, loading, icon: Icon, trend, color = "blue" }) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-xl sm:rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 sm:p-6 shadow-sm transition-all hover:shadow-md hover:-translate-y-1">
      <div className="flex items-start justify-between">
        <div className="space-y-1 sm:space-y-2 min-w-0">
          <p className="text-[10px] sm:text-sm font-medium text-slate-500 dark:text-slate-400 truncate">{title}</p>
          <div className="flex items-baseline gap-1.5 sm:gap-2">
            <h3 className={`text-lg sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white truncate ${loading ? 'animate-pulse bg-slate-100 dark:bg-slate-800 rounded text-transparent' : ''}`}>
              {loading ? '000.00 GB' : value}
            </h3>
            {trend && !loading && (
              <span className={`text-[10px] sm:text-xs font-semibold ${trend > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                {trend > 0 ? '↑' : '↓'} {Math.abs(trend)}%
              </span>
            )}
          </div>
          <p className="text-[9px] sm:text-xs text-slate-400 dark:text-slate-500 line-clamp-1">{description}</p>
        </div>
        
        {Icon && (
          <div className={`rounded-lg sm:rounded-xl border p-1.5 sm:p-2.5 transition-colors shrink-0 ${selectedColor}`}>
